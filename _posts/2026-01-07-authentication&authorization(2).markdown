...
지난 포스트[인증(Authentication)과 인가(Authorization) (1): 개념과 차이점]에서는 인증(Authentication)과 인가(Authorization)의 **개념적인 차이**에 대해 알아보았습니다.
...
이번 포스트에서는 실제 웹 서비스 환경에서 이 두 과정이 **어떤 흐름(Flow)**으로 동작하며, 개발 현장에서 가장 많이 사용되는 **주요 인증 방식**들에 대해 정리해보겠습니다.

<br>

## 1. 인증과 인가의 기본 흐름 (Flow)

사용자, 서비스, 인증 서버 3개의 영역으로 나누어 흐름을 정리했습니다. 그림을 그릴 때 참고하세요.

1.  **사용자 -> 서비스**: 로그인 요청 (ID/PW 주입) `[인증]`
2.  **서비스 -> 인증 서버**: 신원 확인 요청 및 증표 발급 요청 `[인증]`
3.  **인증 서버 -> 서비스**: 신원 확인 완료 및 증표(Token) 발급 `[인증]`
4.  **서비스 -> 사용자**: 증표(Token/Session) 전달 `[인증]`
5.  **사용자 -> 서비스**: 서비스 이용 요청 (Token 제출) `[인가]`
6.  **서비스**: 권한 확인 (Token 유효성 검사 및 인가 처리) `[인가]`
7.  **서비스 -> 사용자**: 최종 응답 (데이터 제공) `[인가]`

<br>

## 2. 대표적인 인증 방식

이 "증표"를 어떻게 관리하느냐에 따라 인증 방식이 나뉩니다. 가장 널리 쓰이는 세 가지 방식을 살펴보겠습니다.
...
"구글로 로그인", "카카오 로그 인"과 같이 **외부 서비스의 계정을 빌려 인증**하는 방식입니다.
