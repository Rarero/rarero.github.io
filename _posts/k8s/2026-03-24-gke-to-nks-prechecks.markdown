...
| # | 질문 | 확인 목적 |
|---|------|----------|
| 1 | 현재 GKE 클러스터 수와 각 클러스터의 **작업 모드**(Autopilot / Standard)는? | Autopilot은 GKE가 내부적으로 설정하는 암묵적 값(리소스 제한, 보안 프로파일, nodeSelector/toleration, QoS 등)이 있어, 전환 시 이를 명시적으로 파악하여 NKS 매니페스트에 반영해야 함 |
| 2 | 각 클러스터에 등록된 **노드풀 수, 노드 스펙(vCPU/Memory), 머신 유형**은? | NKS 노드 스펙 매핑 및 비용 산정 |
| 3 | GKE **Release Channel**(Rapid/Regular/Stable/Extended) 중 어느 채널을 사용 중인가? | 자동 업그레이드 정책 파악. NKS는 수동 업그레이드만 지원하므로, 현재 자동 업그레이드에 의존 중인지 확인 |
| 4 | 현재 GKE 클러스터의 **Kubernetes 버전**은? | NKS 지원 버전과 비교. 버전 갭이 크면 Deprecated API 호환성 이슈 발생 |
...
| 14 | **GKE 자동 주입 사이드카**(Anthos Service Mesh Istio proxy, Cloud SQL Auth Proxy 등) 또는 **Mutating Admission Webhook**이 설정되어 있는가? | 자동 주입되는 사이드카·설정은 NKS 환경에서 수동 구성으로 전환 필요. 매니페스트에 명시되지 않은 암묵적 리소스 존재 여부 확인 |
...
- GKE의 **GKE Gateway Controller**를 통한 네이티브 Gateway API 통합은 NKS에서 사용 불가. 단, **NGINX Gateway Fabric** 등 3rd-party 컨트롤러를 설치하면 Gateway API 자체는 사용 가능.
...
| 출시 시퀀싱 (환경 간 순차 업그레이드) | O | X | 클러스터별 **수동 업그레이드 SOP**(Dev→Staging→Prod 순차)로 관리 |
...
| 기능 | GKE | NKS | 비고 |
|------|:---:|:---:|------|
| VPC-native 클러스터 (Alias IP) | O | △ | NKS도 VPC Subnet 기반 Pod IP 할당으로 유사한 라우팅 가능. 단, Alias IP 방식이 아닌 NCP 자체 구현 |
...
| Gateway API (HTTPRoute, Gateway) | O | △ | GKE Gateway Controller 미지원. **NGINX Gateway Fabric** 등 3rd-party 설치 시 사용 가능 |
...
| 기능 | GKE | NKS | 비고 |
|------|:---:|:---:|------|
| 블록 스토리지 PV | O (Persistent Disk) | O (Block Storage) | CSI 드라이버·StorageClass 재설정 필요. pd-* → NCP Block Storage 클래스 매핑 |
| 관리형 NFS | O (Filestore) | O (NAS) | 마운트 경로·접속 정보 변경. 용량·성능 정책 차이 확인 필요 |
...
| Cloud Pub/Sub | 직접 대응 없음 | Apache Kafka (NKS 위 직접 구축). 단순 이벤트 처리만 필요 시 NCP Cloud Functions + API Gateway 조합 검토 |
