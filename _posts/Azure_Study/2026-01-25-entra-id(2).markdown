...
지난 포스트 [**Azure Entra ID (1): 클라우드 Identity 서비스의 기본 개념과 아키텍처**]({% post_url 2026-01-23-entra-id(1) %})에서는 Entra ID의 기본 구조와 인증 프로토콜을 다뤘다.

이번 포스트에서는 Entra ID의 고급 보안 기능인 Conditional Access, Privileged Identity Management, Identity Protection과 온프레미스 Active Directory와의 하이브리드 통합 시나리오를 심층 분석한다.
...
**Conditional Access**는 ABAC(Attribute-Based Access Control) 모델을 구현하는 Entra ID의 정책 엔진이다. "누가, 어디서, 무엇을, 어떻게" 접근하는지에 따라 동적으로 접근 제어를 수행한다.
...
**PIM**은 Just-In-Time (JIT) 권한 부여를 통해 상시 관리자 권한의 위험을 줄인다.
...
**Identity Protection**은 AI 기반 위험 탐지 및 자동 대응 시스템이다.
...
**역할**: 온프레미스 AD와 Entra ID 간 Identity 동기화.
...
**정의**: 디바이스가 온프레미스 AD와 Entra ID에 동시 가입.
...
이번 포스트에서는 Entra ID의 고급 보안 기능과 하이브리드 Identity 통합을 다뤘다.
...
이로써 접근제어, 디렉토리 서비스, Azure Entra ID에 대한 전체 시리즈를 마무리한다.
...
3. Microsoft Learn, "What is Identity Protection?"
4. Microsoft Learn, "Choose the right authentication method for your Microsoft Entra hybrid identity solution"
5. Microsoft Learn, "What is Microsoft Entra Cloud Sync?"
6. Microsoft Learn, "B2B collaboration overview"
7. Microsoft Learn, "Azure Active Directory B2C documentation"
8. Microsoft Learn, "What is a Primary Refresh Token?"
9. Microsoft Learn, "Plan a Microsoft Entra hybrid join implementation"
10. Microsoft Entra ID documentation
