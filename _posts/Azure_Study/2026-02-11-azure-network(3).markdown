...
1. **NSG 주의**: AGW v2 서브넷에는 인바운드 **65200-65535 포트**(GatewayManager 통신)를 허용하는 NSG 규칙이 필수입니다. 누락 시 AGW가 비정상 상태가 됩니다.
2. **백엔드 Health Probe 경로**: 기본 `/`가 아닌 별도 Health 엔드포인트(`/health`)를 지정해야 불필요한 로직 실행 없이 상태를 확인할 수 있습니다.
3. **Key Vault 인증서 연동**: SSL 인증서를 AGW에 직접 업로드하기보다 **Key Vault에 저장하고 관리 ID로 참조**하면 인증서 갱신이 자동화됩니다.
...
> **실무 팁**: 비-HTTP 프로토콜(게임 서버, IoT, 커스텀 TCP 등)의 글로벌 분산이 필요하면 Traffic Manager가 유일한 선택입니다. HTTP 워크로드라면 대부분 Front Door가 더 적합합니다.
...
| **IDPS** (침입 탐지/방지) | ❌ | ❌ | ✅ |
| **URL 필터링** | ❌ | ✅ | ✅ |
| 웹 카테고리 필터링 | ❌ | ✅ | ✅ (전체 URL 검사) |
| 처리량 | 250 Mbps | 30 Gbps | 100 Gbps |
| 적합 환경 | SMB/소규모 | 일반 엔터프라이즈 | 고보안 요구 금융/공공 |

> **TLS 검사가 필요한 이유**: 암호화된(HTTPS) 트래픽 내부에 숨겨진 악성 페이로드, C2 통신을 탐지하려면 방화벽이 **TLS를 복호화→검사→재암호화**해야 합니다. Premium SKU만 이 기능을 제공합니다.
...
> **실무 팁**: WAF 도입 시 반드시 **Detection 모드로 먼저 운영**하여 오탐을 확인한 후, 정상 트래픽이 차단되는 규칙을 **제외(Exclusion)** 설정한 다음 Prevention 모드로 전환하십시오. 바로 Prevention을 적용하면 정상 사용자가 차단될 수 있습니다.
...
| 공격 텔레메트리 | ❌ | ✅ (Azure Monitor 메트릭, 진단 로그) |
| 공격 알림 | ❌ | ✅ (실시간 알림) |
| 공격 후 보고서 | ❌ | ✅ (상세 완화 보고서) |
| DDoS Rapid Response | ❌ | ✅ (Microsoft DDoS 전문팀 지원) |
| 비용 보호 | ❌ | ✅ (DDoS로 인한 스케일 아웃 비용 크레딧) |
| WAF 할인 | ❌ | ✅ (AGW WAF v2 비용 포함) |

> **비용 보호(Cost Protection)**: DDoS 공격으로 인해 자동 스케일아웃된 리소스(VM, AGW, 대역폭 등) 비용을 Azure 크레딧으로 보상받을 수 있습니다. 이 기능만으로도 대규모 공격 시 예기치 않은 **비용 폭탄을 방지**할 수 있습니다.
...
> **실무 팁**: 미션 크리티컬 워크로드에서는 ExpressRoute를 **기본 경로**로, VPN Gateway S2S를 **백업 경로(Failover)**로 구성하는 하이브리드 방식을 권장합니다. ExpressRoute 회선 장애 시 VPN으로 자동 전환됩니다.
...
> **실무 팁**: `NSG 흐름 로그 + 트래픽 분석`을 조합하면 **어떤 IP가 어느 포트로 얼마나 통신하는지** Log Analytics 대시보드에서 시각적으로 확인할 수 있습니다. 이상 행위 탐지, 네트워크 설계 최적화, 보안 감사 보고서 작성에 핵심 도구입니다.
