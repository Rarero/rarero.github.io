...
> - **플랜(Plan)**: Defender for Cloud 내에서 활성화하는 보호 단위
> - **기능(Feature)**: 특정 플랜 안에서 제공되는 세부 기능
> - **에이전트(Agent)**: 워크로드에 배포되어 데이터를 수집하는 소프트웨어
...
**Microsoft Defender for Cloud**는 Azure, AWS, GCP, 온프레미스를 아우르는 **클라우드 네이티브 보안 플랫폼(CNAPP)**입니다. 하나의 서비스 안에 세 가지 핵심 축이 존재합니다.
...
|---|---|---|
| **Secure Score** | ✅ | ✅ |
| **보안 권장 사항** | 기본 권장 사항 | 확장 권장 사항 + 공격 경로 분석 |
| **자산 인벤토리** | ✅ | ✅ |
| **취약점 평가** | ❌ | ✅ (MDVM 기반) |
| **위협 탐지 알림** | ❌ | ✅ (플랜별) |
| **에이전트리스 스캔** | ❌ | ✅ |
