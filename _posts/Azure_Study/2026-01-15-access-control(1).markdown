...
- **벨-라파듈라(Bell-LaPadula) 모델**: **기밀성(Confidentiality)** 유지에 초점을 맞춘 모델입니다. 정보가 밖으로 새나가는(유출) 것을 막는 것이 목표입니다.
  - **No Read Up (상위 읽기 금지)**: 낮은 등급의 사용자는 높은 등급의 문서를 볼 수 없습니다.
    - *이유: 일반 사병이 장군의 1급 기밀 문서를 보면 안 되기 때문입니다.*
  - **No Write Down (하위 쓰기 금지)**: 높은 등급의 사용자는 낮은 등급의 문서에 정보를 기록할 수 없습니다.
    - *이유: 1급 기밀 취급자가 실수로 '1급 정보'를 '공개 게시판(낮은 등급)'에 적어서 정보가 줄줄 새는 사고를 막기 위함입니다.*

- **비바 무결성(Biba Integrity) 모델**: **무결성(Integrity)** 유지에 초점을 맞춘 모델입니다. 잘못된 데이터가 시스템을 망가뜨리는(오염) 것을 막는 것이 목표입니다.
  - **No Write Up (상위 쓰기 금지)**: 낮은 등급의 사용자는 높은 등급의 문서를 수정할 수 없습니다.
    - *이유: 일반 사용자가 시스템의 중요 설정 파일(높은 등급)을 함부로 고쳐서 고장 내는 것을 막기 위함입니다.*
  - **No Read Down (하위 읽기 금지)**: 높은 등급의 사용자는 낮은 등급의 데이터를 읽을(신뢰할) 수 없습니다.
    - *이유: 시스템 관리자(높은 등급)가 검증되지 않은 인터넷 상의 스크립트(낮은 등급)를 가져와서 실행하면, 시스템 전체가 바이러스에 감염되거나 오염될 수 있기 때문입니다.*
...
리소스의 **수명 주기와 구성(Configuration)을 관리**하는 영역입니다. 예를 들어 스토리지 계정을 생성하거나, 가상 머신을 재시작하고, 방화벽 규칙을 업데이트하는 작업들이 여기에 속합니다. 이러한 작업은 주로 Azure Resource Manager(ARM)와 같은 관리 API를 통해 수행됩니다.

**2) 데이터 평면 (Data Plane)**
리소스 내부에 저장된 **실제 데이터와 상호 작용**하는 영역입니다. 스토리지 계정 내부의 파일을 다운로드하거나, SQL 데이터베이스의 테이블을 조회하고, Event Hub로 메시지를 전송하는 등의 작업이 해당됩니다.
