...
이 분산 알고리즘은 **부하분산 규칙(Load balancing rules)** 편집 화면에서 **Session persistence** 항목으로 선택할 수 있습니다. Inbound NAT rules / Outbound rules / Gateway Load Balancer 화면에서는 이 옵션이 노출되지 않습니다.
...
> **팁**: 레거시 애플리케이션이 세션 상태를 서버 메모리에 저장하는 경우 Client IP를 쓰면 세션이 유지됩니다. 가능하면 앱을 stateless하게 만들고 기본값(None)을 사용하는 것이 부하 분산 효과가 더 좋습니다.

### Standard Load Balancer vs Gateway Load Balancer

| 항목 | Standard Load Balancer | Gateway Load Balancer |
|---|---|---|
| 주 목적 | 애플리케이션 트래픽 분산 | NVA(방화벽/IDS 등) 서비스 체이닝 |
| 대표 사용 위치 | 웹/API/내부 서비스 앞단 | 트래픽 검사 장비 앞단 |
| 동작 방식 | Frontend IP로 유입된 트래픽을 백엔드로 분산 | 트래픽을 NVA로 투과 전달 후 원경로 복귀 |
| 규칙 중심 | 부하분산 규칙, NAT 규칙, 아웃바운드 규칙 | 보안/검사 경유 중심 구성 |
| 적합한 경우 | 일반적인 L4 부하분산 | 중앙 보안 장비 삽입이 필요한 구조 |
...
| 기능 | 실제 동작 | 사용 예시 |
|---|---|---|
| **URL 경로 기반 라우팅** | `/api/*`는 API 풀, `/images/*`는 정적 풀로 보내고, 경로별로 서로 다른 HTTP Settings(타임아웃/포트/프로토콜) 적용 가능 | 하나의 도메인에서 웹/API/정적 자원을 분리하고 경로별 특성에 맞게 튜닝할 때 |
...
| **SSL/TLS 종료** | 클라이언트 TLS를 AGW에서 종료하고 백엔드에는 HTTP 또는 재암호화 HTTPS로 전달 | 백엔드 암복호화 부담을 줄이고, 인증서 갱신/배포를 AGW에서 중앙 관리할 때 |
...
| **상태 판별(Health Probe)** | 백엔드 상태를 주기적으로 확인해 비정상 인스턴스를 자동 제외 | 일부 서버 장애 시에도 서비스 중단 없이 트래픽을 우회할 때 |
...
| **Weighted** | 가중치 비율로 분산 | 블루-그린 배포, 점진적 마이그레이션 |
...
> **실수하기 쉬운 포인트**: DNAT 규칙이 가장 먼저 평가됩니다. DNAT로 허용된 트래픽은 Network 규칙에서 **암묵적으로 허용**됩니다(별도 Network 규칙 불필요). "왜 DNAT 트래픽에 대한 Network 규칙이 안 먹히지?" 하는 경우가 종종 있는데, 이 동작 때문입니다.
...
WAF를 처음 도입할 때는 반드시 **Detection 모드**로 시작하세요. 정상 요청이 오탐으로 차단될 수 있으므로, 로그를 확인해 **Exclusion(제외)** 설정을 적용한 뒤 Prevention 모드로 전환하는 것이 권장됩니다.
