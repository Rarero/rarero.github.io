...
지난 포스트 [**디렉토리 서비스(Directory Service) (2): Active Directory 심층 분석**]({% post_url 2026-01-21-directory-service(2) %})에서는 온프레미스 Active Directory의 내부 동작을 다뤘습니다.

이번 포스트에서는 Microsoft의 클라우드 Identity 플랫폼인 **Microsoft Entra ID** (구 Azure Active Directory)의 기본 개념, 아키텍처, 핵심 프로토콜을 분석합니다.
...
**Microsoft Entra ID**는 클라우드 기반 Identity 및 Access Management (IAM) 서비스입니다. 2023년 7월 Azure Active Directory에서 Microsoft Entra ID로 브랜드가 변경되었습니다.
...
**중요**: Entra ID는 AD의 "클라우드 버전"이 아닙니다. 완전히 다른 설계 철학과 프로토콜을 가진 독립적인 서비스입니다.
...
**정의**: Entra ID의 기본 격리 단위. 조직을 대표하는 전용 인스턴스입니다.
...
**정의**: 관리 범위를 제한하는 논리적 컨테이너. Active Directory의 OU와 유사하지만 더 제한적입니다.
...
**역할**: 인가(Authorization) 프레임워크. 서드파티 앱이 사용자 리소스에 접근할 수 있도록 권한 위임합니다.
...
**역할**: OAuth 2.0 위에 구축된 인증(Authentication) 레이어입니다.
...
**정의**: Entra ID 및 Microsoft 365 리소스에 접근하는 통합 REST API입니다.
...
다음 포스트에서는 Entra ID의 고급 기능인 Conditional Access, Privileged Identity Management, Identity Protection, 그리고 온프레미스 AD와의 하이브리드 통합 시나리오를 상세히 다룰 예정입니다.
